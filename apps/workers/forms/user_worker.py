...
from apps.workers.models import WorkerBasic, Worker, WorkerClosed, Subdivision, Department, Chief
...
class Search_User_Filter(forms.Form):
    """Форма поиска. пользователь расширенная."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        SUBDIVISION = [(i.slug, i.name) for i in Subdivision.objects.all().only('name', 'slug').order_by('name')]
        SUBDIVISION.insert(0, ('', 'Все управления'))
        self.fields['subdivision'] = forms.ChoiceField(label='Управления',
                                                       required=False,
                                                       choices=SUBDIVISION,
                                                       widget=forms.Select(
                                                           attrs={'class': 'select2', 'style': 'width: 100%'}))
        DEPARTMENT = [(i.slug, i.name) for i in Department.objects.all().only('name', 'slug').order_by('name')]
        DEPARTMENT.insert(0, ('', 'Все отделы'))
        self.fields['department'] = forms.ChoiceField(label='Отделы',
                                                      required=False,
                                                      choices=DEPARTMENT,
                                                      widget=forms.Select(
                                                          attrs={'class': 'select2', 'style': 'width: 100%'}))
        CHIEF = [(i.slug, i.name) for i in Chief.objects.all().only('name', 'slug').order_by('rights')]
        CHIEF.insert(0, ('', 'Все должности'))
        self.fields['chief'] = forms.ChoiceField(label='Должности',
                                                 required=False,
                                                 choices=CHIEF,
                                                 widget=forms.Select(
                                                     attrs={'class': 'select2', 'style': 'width: 100%'}))
